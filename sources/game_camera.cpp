...
GameCamera::GameCamera(EntityWorld &world, sf::Vector2f position, sf::Vector2f cameraSize)
    : _world(&world)
{
    const Entity camera = _world->create();
    _world->emplace<KinematicPhysicsObjectPrototype>(camera, position, cameraSize);
