...
void Render(EntityWorld &world, sf::RenderTarget &renderTarget) {
    world.view<const PositionComponent, SpriteComponent>()
            .each([&renderTarget](const PositionComponent position, SpriteComponent &sprite) {
                sprite.shape.setPosition({position.x, position.y});
            });
    world.view<const SpriteComponent>()
            .each([&renderTarget](const SpriteComponent &sprite) {
                renderTarget.draw(sprite.shape);
            });
}
